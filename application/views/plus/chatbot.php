			...
			<?php if($reply->reply_from == 1 ){ ?>
				...
				<?php if(count($records) == $ctr) { ?>
					...
					<?php $suggests = explode(',',$reply->reply_suggested);?>
						...
						<div class="ul_section_full">
							<ul class="ul_msg">
								<li style="font-size: 13px; color: #010101;">Choose your reply below</li>
								<?php foreach($suggests as $suggest){ ?>
									<?php foreach($replies as $reply){ ?>
										<?php if($reply->reply_id == intval($suggest)){ ?>
											<li style="font-size: 13px; color: #010101;">
												<!-- i class="ti-angle-double-right">&nbsp; &nbsp;</i -->
												<a href="#" style="margin: 5px!important; text-transform: none !important; " class="btn btn-info btn-sm" role="button" onclick="set_reply(<?php echo $reply->reply_id; ?>)"><?php echo $reply->reply; ?></a>
											</li>
										<?php break; } ?>
									<?php } ?>
								<?php } ?>
